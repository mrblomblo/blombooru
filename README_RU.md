...
> Актуально на момент: 09 сен 2026
...
- **Автоматическое тегирование:** Быстрая маркировка с помощью интеграции WDv3 Auto Tagger, анализирующей изображения и предлагающей точные теги одним кликом. Поддерживает опциональное аппаратное ускорение (только на Nvidia видеокартах)
...
|:-------|:----------|:---------|
| **Последний стабильный** | `latest` (по умолчанию) | общее применение, отслеживает последний релиз с GitHub |
| **Последний стабильный (CUDA)** | `latest-cuda` | общее применение, отслеживает последний релиз с GitHub с CUDA-ускорением |
| **Pre-release** | `pre` | тестирование будущих версий, отслеживает последний pre-release с GitHub |
| **Pre-release (CUDA)** | `pre-cuda` | тестирование будущих версий, отслеживает последний pre-release с GitHub с CUDA-ускорением |
...
1. **Подготовка файлов**
    ...
    Создайте папку для Blombooru (к примеру, `blombooru`), затем загрузите файлы `docker-compose.yml` и `example.env` с [последнего релиза](https://github.com/mrblomblo/blombooru/releases/latest) и положите их в эту папку. Если вы планируете использовать GPU аппаратное ускорение, скачайте также и `hwaccel.yml`
...
Если у вас есть видеокарта Nvidia и вы хотите значительно ускорить работу WDv3, вы можете использовать Docker образ с CUDA-ускорением. 

> [!IMPORTANT]
> На хост-машине должен быть установлен Nvidia Container Toolkit, также должны быть установлены драйверы последней версии. Само собой, видеокарта должна быть более-менее свежей серии.

Установка:

1. **Загрузите файл hwaccel.yml**  
    После загрузки поместите `hwaccel.yml` в папке рядом с `docker-compose.yml` и `.env`.

2. **Отредактируйте docker-compose.yml**  
    Откройте `docker-compose.yml`, перейдите к строке web service, и уберите три `#` символа с начала блока `extends:`:
...
3. **Сконфигурируйте переменную среды**  
    В вашем `.env` файле, убедитесь что настройка tagger device установлена в `auto` (по умолчанию) или `cuda`:
...
4. **Запустите контейнер**  
    Стяните новый CUDA образ и запустите контейнер:
...
> [!WARNING]
> Если вы используете latest-cuda образ, вам необходимо раскоментить `extends:` в `docker-compose.yml`. Если вы используете CUDA-образ но не пробрасываете видеокарту в контейнер, Blombooru крашнется при попытке загрузки AI модели.

Переменная среды `BLOMBOORU_WD_TAGGER_DEVICE` контроллирует инициализацию теггера:

- `auto` (по умолчанию): Использует видеокарту если установлен пакет `onnxruntime-gpu`, в противном случае работает на CPU.
- `cuda`: Строго требует использования видеокарты. Если пакет не установлен, Blombooru выкинет ошибку и не запустится.
- `cpu`: Требует использования CPU, даже если доступна видеокарта.
...
> [!NOTE]
> Если вы планируете использовать видокарту Nvidia для AI предсказания тегов, установите CUDA requirements файл (`requirements-cuda.txt`) вместо обычного (`requirements.txt`). Также необходимо установить подходящий драйвер и CUDA toolkit.
