...
Если вам требуется, чтобы несколько инстансов Blombooru имели одну и ту же БД тегов (таким образом, теги созданные в одном инстансе станут доступны в других), вы можете включить опцию **Общей БД тегов**:

1. **Запустите контейнер общей БД:**
   ```bash
   docker compose -f docker-compose.shared-tags.yml up -d
   ```

2. **Настройте каждый из инстансов Blombooru:**
